...
namespace TSPSolver
{
  public class TSPBranch
  {
    private const double Tolerance = 1e-5;
    private readonly TSPRoute[][] _routes;

    public TSPBranch(TSPRoute[][] routes) : this(routes, 0)
    {
    }

    private TSPBranch(TSPRoute[][] routes, double lowBound)
    {
    ...
    public TSPRoute CurrentRoute { get; private set; }
    ...
    private Tuple<int, int> FindMaxPenyIndices()
    {
    ...
    public TSPBranch[] GetSubBranches()
    {
      Tuple<int, int> slicePoint = FindMaxPenyIndices();

      TSPBranch left = GetLeftBranch(slicePoint);
      left.CurrentRoute = _routes[slicePoint.Item1][slicePoint.Item2];

      TSPBranch right = GetRightBranch(slicePoint);

      return new[] {left, right};
    }

    private TSPBranch GetLeftBranch(Tuple<int, int> slicePoint)
    {
      ...
      return new TSPBranch(newRoutes, LowBound);
    }

    private TSPBranch GetRightBranch(Tuple<int, int> slicePoint)
    {
